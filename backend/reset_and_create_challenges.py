...
try:
    ...
    now = datetime.now()
    end_time = now + timedelta(hours=24)  # Active for 24 hours
    
    # Create 3 new challenges - one per level
    challenges_data = [
        {
            "level": 1,
    ...
    for challenge_data in challenges_data:
        ...
        for i, fragment_content in enumerate(challenge_data["fragments"]):
            if fragment_content.strip():  # Skip empty lines
        ...
        print(f"✅ Created {challenge_data['title']}")
    
    db.commit()
    print("\n🎉 Successfully created 3 new challenges!")
    print("📊 One challenge per level (1, 2, 3)")
