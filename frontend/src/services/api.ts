...
class ApiClient {
  ...
  async getUserProgress(userId: string): Promise<UserProgress> {
    const response = await fetch(`${this.baseUrl}/api/auth/users/${userId}/progress`);
    if (!response.ok) throw new Error('Failed to fetch user progress');
    return response.json();
  }

  async completeLevel(userId: string, level: number, timeTaken: number = 300): Promise<any> {
    const response = await fetch(`${this.baseUrl}/api/auth/users/${userId}/complete-level/${level}?time_taken=${timeTaken}`, {
      method: 'POST',
    });
    if (!response.ok) {
      const error = await response.json();
  ...
  async getAllUsers(adminId: string): Promise<any[]> {
    const response = await fetch(`${this.baseUrl}/api/admin/users?admin_id=${adminId}`);
    if (!response.ok) throw new Error('Failed to fetch users');
    return response.json();
  }

  async getChallengesByLevel(level: number, adminId: string): Promise<any[]> {
    const response = await fetch(`${this.baseUrl}/api/admin/challenges/by-level/${level}?admin_id=${adminId}`);
    if (!response.ok) throw new Error('Failed to fetch challenges');
    return response.json();
  }

  async deleteChallenge(challengeId: string, adminId: string): Promise<any> {
    const response = await fetch(`${this.baseUrl}/api/admin/challenges/${challengeId}?admin_id=${adminId}`, {
      method: 'DELETE',
    });
  ...
  async getAdminStats(adminId: string): Promise<any> {
    const response = await fetch(`${this.baseUrl}/api/admin/stats?admin_id=${adminId}`);
    if (!response.ok) throw new Error('Failed to fetch stats');
    return response.json();
  }

  async createAdmin(username: string, email: string, password: string): Promise<any> {
    const response = await fetch(`${this.baseUrl}/api/admin/create-admin?username=${username}&email=${email}&password=${password}`, {
      method: 'POST',
    });
    if (!response.ok) {
      const error = await response.json();
  ...
  async submitCode(challengeId: string, participantId: string, code: string): Promise<Submission> {
  ...
  async getSubmissionsByChallenge(challengeId: string): Promise<Submission[]> {
    const response = await fetch(`${this.baseUrl}/api/submissions/challenge/${challengeId}`);
    if (!response.ok) throw new Error('Failed to fetch submissions');
    return response.json();
  }

  async getSubmissionsByParticipant(participantId: string): Promise<Submission[]> {
    const response = await fetch(`${this.baseUrl}/api/submissions/participant/${participantId}`);
    if (!response.ok) throw new Error('Failed to fetch submissions');
    return response.json();
  }
